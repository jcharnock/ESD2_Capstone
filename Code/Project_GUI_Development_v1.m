...
eZ.ValueChangedFcn  = @(src,~) setIfValid(sZ,'Value',src.Value);
    ...
    function onTrackServe()
        ...
        safeStatus('Tracking 1 serve...');
        drawnow limitrate;
    
        servePresets = ["serve_in_1","serve_in_2","serve_out_1","serve_out_2","serve_net_1","serve_net_2"];
        ...
        for i = 1:numel(servePresets)
            ...
            shot = captureShotTrajectory(tVec, servePresets(i));
            ...
            results{i,1} = char(servePresets(i));
            ...
            plotSingleShotCourt(shot, char(servePresets(i)), COURT);
        ...
        shotSummary.Text = 'Tracked 1 serve';
    ...
    function onTrackVolley()
        ...
        for i = 1:numel(volleyPresets)
            ...
            plotSingleShotCourt(shot, char(volleyPresets(i)), COURT);
    ...
    function decision = classifyShotFromBounce(shotType, bounceXYZ, courtGeom)
        ...
        xL = -courtGeom.singlesHalfWidth;
        xR =  courtGeom.singlesHalfWidth;
    
        switch lower(char(shotType))
            case 'serve'
                inX = (x >= xL) && (x <= xR);
                inY = (y >= courtGeom.serviceBoxYMin) && (y <= courtGeom.serviceBoxYMax);
    
            case 'volley'
                inX = (x >= xL) && (x <= xR);
                inY = (y >= courtGeom.farCourtYMin) && (y <= courtGeom.farCourtYMax);
    
            otherwise
                [in,on] = inpolygon(x, y, courtGeom.polyX, courtGeom.polyY);
                if in || on
                    decision = "IN";
                else
                    decision = "OUT";
    ...
    function preset = getShotPreset(name, groundZ)
        ...
        switch lower(char(name))
            ...
            case 'serve_out_1'   % too wide
                preset.x0 = 0.0;  preset.y0 = -11.0; preset.z0 = 2.6;
                preset.vx = 5.8;  preset.vy = 18.0;  preset.vz = 1.8;
                preset.bounceT = 0.80;
    
            case 'serve_out_2'   % too deep
                preset.x0 = 0.0;  preset.y0 = -11.0; preset.z0 = 2.6;
                preset.vx = 0.1;  preset.vy = 21.5;  preset.vz = 1.8;
                preset.bounceT = 0.86;
    ...
    function plotSingleShotCourt(shot, shotName, courtGeom)
        ...
        if all(isfinite(plotXYZ))
            ...
            plot(ax, bx, by, mk, 'MarkerSize', 10, 'LineWidth', 2);
